-- rpc_place_order_atomic: SSOT 라운딩 + 평단(수수료 포함) + 실현손익 적용
...
begin
  ...
  -- SSOT 라운딩
  v_subtotal_krw := public.fn_round_krw(v_price * v_qty);
  v_fee_krw := public.fn_fee_krw(v_subtotal_krw, p_fee_rate);

  if p_side = 'buy' then
    v_total_krw := v_subtotal_krw + v_fee_krw;
  else
    v_total_krw := v_subtotal_krw - v_fee_krw;
    if v_total_krw < 0 then raise exception 'INVALID_TOTAL'; end if;
  end if;

  -- SELL: 보유수량 + 평단 조회(실현손익 계산용)
  if p_side = 'sell' then
    select quantity, coalesce(avg_price_krw, avg_price, 0) into v_pos_qty, v_old_avg
    from positions
  ...
  -- 체결 생성 (subtotal_krw, fee_krw, total_krw, realized_pnl_krw)
  if p_side = 'buy' then
    insert into trades (user_id, product_id, price_at_trade, quantity, amount, type, subtotal_krw, fee_krw, total_krw)
    values (p_user_id, v_product_id, v_price, v_qty, v_total_krw, 'BUY', v_subtotal_krw, v_fee_krw, v_total_krw)
    returning id into v_trade_id;
  else
    v_realized_pnl := v_total_krw - (v_old_avg * v_qty);
    insert into trades (user_id, product_id, price_at_trade, quantity, amount, type, subtotal_krw, fee_krw, total_krw, realized_pnl_krw)
    values (p_user_id, v_product_id, v_price, v_qty, v_total_krw, 'SELL', v_subtotal_krw, v_fee_krw, v_total_krw, v_realized_pnl)
    returning id into v_trade_id;
  end if;
  ...
  if p_side = 'buy' then
    ...
    values
      (p_user_id, v_order_id, 'CASH_DEBIT', 'KRW', -v_total_krw, null, 0, 'PRODUCT_PURCHASE', '{}'::jsonb),
      (p_user_id, v_order_id, 'ASSET_CREDIT', 'KRW', 0, p_item_id, v_qty, 'PRODUCT_PURCHASE', '{}'::jsonb);

    -- positions는 fn_update_position_on_ledger 트리거로 갱신 (avg_price = total_cost/qty, 수수료 포함)
    ...
    if abs(v_cash_debit_sum - v_total_krw) > 0.0001 then raise exception 'ENGINE_MISMATCH_CASH'; end if;
    ...
    return json_build_object(
      ...
      'side', p_side, 'subtotal_krw', v_subtotal_krw, 'fee_krw', v_fee_krw,
      'total_krw', v_total_krw, 'idempotency_key', p_idempotency_key
    );
    ...
    insert into ledger_entries (user_id, order_id, entry_type, currency, amount, asset_id, quantity, memo, metadata)
    values
      (p_user_id, v_order_id, 'CASH_CREDIT', 'KRW', v_total_krw, null, 0, 'PRODUCT_SELL', '{}'::jsonb),
    ...
    if abs(v_cash_credit_sum - v_total_krw) > 0.0001 then raise exception 'ENGINE_MISMATCH_CASH_SELL'; end if;

    return json_build_object(
      'ok', true, 'deduped', false, 'order_id', v_order_id, 'trade_id', v_trade_id,
      'side', p_side, 'subtotal_krw', v_subtotal_krw, 'fee_krw', v_fee_krw,
      'net_credit_krw', v_total_krw, 'realized_pnl_krw', v_realized_pnl, 'idempotency_key', p_idempotency_key
    );
  end if;
