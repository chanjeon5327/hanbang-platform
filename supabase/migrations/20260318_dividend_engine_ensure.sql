-- PATCH: dividend engine ensure (rpc 미존재 시 스키마 캐시/마이그레이션 누락 대응)
-- Supabase SQL Editor 또는 db push로 적용
...
-- 2) dividend_distributions + unique
...
CREATE UNIQUE INDEX IF NOT EXISTS dividend_distributions_dividend_user_uniq
  ON public.dividend_distributions (dividend_id, user_id);
...
DROP POLICY IF EXISTS "dividends_select_all" ON public.dividends;
CREATE POLICY "dividends_select_all" ON public.dividends FOR SELECT USING (true);
DROP POLICY IF EXISTS "dividend_distributions_select_own" ON public.dividend_distributions;
CREATE POLICY "dividend_distributions_select_own" ON public.dividend_distributions FOR SELECT USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "positions_snapshot_select_own" ON public.positions_snapshot;
CREATE POLICY "positions_snapshot_select_own" ON public.positions_snapshot FOR SELECT USING (auth.uid() = user_id);

-- 5) user_positions view
CREATE OR REPLACE VIEW public.user_positions AS
SELECT user_id, asset_id AS item_id,
  COALESCE(SUM(CASE WHEN entry_type = 'ASSET_CREDIT' THEN quantity WHEN entry_type = 'ASSET_DEBIT' THEN -quantity ELSE 0 END), 0) AS total_quantity
...
DECLARE v_total_dividend numeric; v_dividend_id uuid;
BEGIN
...
-- 7) rpc_execute_dividend (idempotent)
CREATE OR REPLACE FUNCTION public.rpc_execute_dividend(p_dividend_id uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  r RECORD; v_total_shares numeric; v_dividend_per_share numeric; v_total_dividend numeric;
  v_item_id uuid; v_dist_id uuid;
BEGIN
  SELECT d.item_id, d.total_dividend_amount INTO v_item_id, v_total_dividend FROM dividends d WHERE d.id = p_dividend_id;
  IF v_item_id IS NULL THEN RETURN jsonb_build_object('ok', false, 'error', 'DIVIDEND_NOT_FOUND'); END IF;
  SELECT COALESCE(SUM(total_quantity), 0) INTO v_total_shares FROM user_positions WHERE item_id = v_item_id;
  IF v_total_shares <= 0 THEN RETURN jsonb_build_object('ok', true, 'distributed_count', 0); END IF;
  v_dividend_per_share := v_total_dividend / v_total_shares;
  FOR r IN SELECT up.user_id, up.item_id, up.total_quantity FROM user_positions up WHERE up.item_id = v_item_id AND up.total_quantity > 0
  LOOP
    v_dist_id := NULL;
    ...
    ON CONFLICT (dividend_id, user_id) DO NOTHING RETURNING id INTO v_dist_id;
    IF v_dist_id IS NOT NULL THEN
      ...
      VALUES (r.user_id, NULL, 'CASH_CREDIT', 'KRW', ROUND(r.total_quantity * v_dividend_per_share, 0), NULL, 0, 'DIVIDEND', jsonb_build_object('dividend_id', p_dividend_id, 'item_id', v_item_id));
    END IF;
  END LOOP;
