...
export type MarketDetailLike = Record<string, unknown> & {
  ...
  summary?: string
  overview?: string
  investment_points?: string[] | string
  risk_points?: string[] | string
  roadmap_items?: string[] | string
  creator_plan?: {
    summary?: string
    overview?: string
    target_audience?: string
...
function getSummary(item: MarketDetailLike, fallback: string): string {
  return (
    safeString(item.creator_plan?.summary) ||
    safeString(item.summary) ||
    safeString(item.overview) ||
    fallback
  )
}

function getTargetAudience(item: MarketDetailLike, fallback: string): string {
  return (
    safeString(item.creator_plan?.target_audience) ||
    safeString(item.target_audience) ||
    fallback
  )
}

function getRevenueModel(item: MarketDetailLike, fallback: string): string {
  return (
    safeString(item.creator_plan?.revenue_model) ||
    safeString(item.revenue_model) ||
    fallback
  )
}

function getInvestmentPoints(item: MarketDetailLike, fallback: string[]): string[] {
  const next =
    toStringList(item.creator_plan?.investment_points) ??
    toStringList(item.investment_points)
  return next.length > 0 ? next : fallback
}

function getRiskPoints(item: MarketDetailLike, fallback: string[]): string[] {
  const next =
    toStringList(item.creator_plan?.risk_points) ??
    toStringList(item.risk_points)
  return next.length > 0 ? next : fallback
}

function getRoadmapItems(item: MarketDetailLike, fallback: string[]): string[] {
  const next =
    toStringList(item.creator_plan?.roadmap_items) ??
    toStringList(item.roadmap_items)
  return next.length > 0 ? next : fallback
}
...
function buildDefaultTemplate(item: MarketDetailLike): DetailTemplate {
  ...
  const creator = getCreator(item)
  const price = getPriceLabel(item)
  const fallbackOneLiner = `${title}는 ${creator} 중심의 성장형 IP 자산으로, 시장 거래 흐름과 콘텐츠 확장 가능성을 함께 보는 구조입니다.`

  return {
    category: 'default',
    categoryLabel: 'IP 자산',
    oneLiner: getSummary(item, fallbackOneLiner),
    spotlightTitle: '핵심 요약',
    spotlightItems: [
      { label: '운영 주체', value: creator },
      { label: '기준 가격', value: price },
      { label: '플랫폼', value: getPlatform(item) },
      { label: '타깃층', value: getTargetAudience(item, '확장형 대중 타깃') },
    ],
    investmentPoints: getInvestmentPoints(item, defaultFallbackInvestment),
    riskPoints: getRiskPoints(item, defaultFallbackRisk),
    revenueTitle: '기본 수익 구조',
    revenueDescription: getRevenueModel(
      item,
      '광고, 판매, 제휴, 라이선싱, 팬 기반 부가수익 등 복합 구조로 확장 가능한 자산으로 해석합니다.'
    ),
    recentPerformance: buildCommonRecentPerformance(item),
    roadmap: getRoadmapItems(item, buildCommonRoadmap(item)),
  }
}

const youtubeFallbackInvestment = [
  '콘텐츠 포맷이 명확하면 재생산성과 시리즈화 가능성이 높습니다.',
  '광고, 협찬, 멤버십, 커머스 연동 등 수익화 채널이 다층적으로 열릴 수 있습니다.',
  '팬 충성도가 높을수록 신규 포맷 전환과 외부 브랜드 협업이 유리합니다.',
]
const youtubeFallbackRisk = [
  '플랫폼 알고리즘 변화나 추천 노출 감소가 성과를 흔들 수 있습니다.',
  '업로드 주기 불안정은 조회수와 팬 리텐션 저하로 이어질 수 있습니다.',
  '특정 포맷 의존도가 높으면 장기적인 성장 탄력성이 떨어질 수 있습니다.',
]
const youtubeFallbackRoadmap = [
...
function buildYoutubeTemplate(item: MarketDetailLike): DetailTemplate {
  ...
  const fallbackOneLiner = `${title}는 ${creator}가 운영하는 영상형 IP로, 조회수·팬 반응·업로드 지속성이 수익성과 재평가 포인트가 되는 구조입니다.`
  ...
  return {
    ...
    investmentPoints: getInvestmentPoints(item, youtubeFallbackInvestment),
    riskPoints: getRiskPoints(item, youtubeFallbackRisk),
...
function buildWebtoonTemplate(item: MarketDetailLike): DetailTemplate {
  ...
  const fallbackOneLiner = `${title}는 ${creator} 중심의 서사형 IP로, 연재 지속성·유료 전환율·2차 창작 및 영상화 가능성이 핵심 가치입니다.`
  ...
  return {
    ...
    spotlightItems: [
      ...
      { label: '주독자층', value: getTargetAudience(item, '장르 팬덤 중심') },
    ],
    investmentPoints: getInvestmentPoints(item, webtoonFallbackInvestment),
    riskPoints: getRiskPoints(item, webtoonFallbackRisk),
...
function buildWebnovelTemplate(item: MarketDetailLike): DetailTemplate {
  ...
  const fallbackOneLiner = `${title}는 ${creator}가 구축한 텍스트 기반 세계관 자산으로, 독자 락인과 시즌 누적, 2차 콘텐츠 확장이 핵심 가치입니다.`
  ...
  return {
    ...
    spotlightItems: [
      ...
      { label: '연재 플랫폼', value: platform },
      { label: '회차 수', value: episodeCount },
      { label: '팬 특성', value: getTargetAudience(item, '정주행형 독자층') },
    ],
    investmentPoints: getInvestmentPoints(item, webnovelFallbackInvestment),
    riskPoints: getRiskPoints(item, webnovelFallbackRisk),
...
function buildMusicTemplate(item: MarketDetailLike): DetailTemplate {
  ...
  const fallbackOneLiner = `${title}는 ${creator} 중심의 음악·공연형 IP로, 팬덤 밀도와 컴백·공연 이벤트가 가치 재평가를 이끄는 구조입니다.`
  ...
  return {
    ...
    spotlightItems: [
      ...
      { label: '팬덤 성격', value: getTargetAudience(item, fandom) },
      { label: '예정 일정', value: schedule },
    ],
    investmentPoints: getInvestmentPoints(item, musicFallbackInvestment),
    riskPoints: getRiskPoints(item, musicFallbackRisk),
    ...
    roadmap: getRoadmapItems(item, musicFallbackRoadmap),
  }
}

const sportsFallbackInvestment = [
...
function buildSportsTemplate(item: MarketDetailLike): DetailTemplate {
  ...
  const fallbackOneLiner = `${title}는 ${creator} 중심의 시즌·이벤트 반응형 IP로, 경기·출연·화제성 이슈가 가치 변동의 핵심 동인입니다.`
  ...
  return {
    ...
    spotlightItems: [
      { label: '프로젝트', value: title },
      { label: '주체', value: creator },
      { label: '팬 반응', value: getTargetAudience(item, fandom) },
      { label: '시즌 일정', value: schedule },
    ],
    investmentPoints: getInvestmentPoints(item, sportsFallbackInvestment),
    riskPoints: getRiskPoints(item, sportsFallbackRisk),
    ...
    roadmap: getRoadmapItems(item, sportsFallbackRoadmap),
  }
}
