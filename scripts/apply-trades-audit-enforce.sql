...
DROP TRIGGER IF EXISTS trg_trades_require_audit ON trades;

CREATE TRIGGER trg_trades_require_audit
BEFORE INSERT OR UPDATE ON trades
FOR EACH ROW
EXECUTE FUNCTION public.fn_require_financial_audit();

-- rpc_write_financial_audit 4-param overload (system action, user_id NULL)
...
BEGIN
  ...
  VALUES (NULL, p_action, p_target_type, p_target_id, p_metadata);
EXCEPTION WHEN OTHERS THEN
  NULL;
END;
$$;

GRANT EXECUTE ON FUNCTION public.rpc_write_financial_audit(text, text, text, jsonb) TO service_role;
GRANT EXECUTE ON FUNCTION public.rpc_write_financial_audit(text, text, text, jsonb) TO authenticated;

-- rpc_match_orders: set_config + audit 강제
    ...
    LOOP
      ...
      IF v_inserted THEN
        ...
        BEGIN
          ...
          VALUES (v_bid.user_id, v_trade_id, 'ASSET_CREDIT', 'KRW', 0, p_item_id, v_match_qty, 'TRADE_BUY', jsonb_build_object('trade_id', v_trade_id)),
