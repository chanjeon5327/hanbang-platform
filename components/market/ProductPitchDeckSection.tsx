...
export default function ProductPitchDeckSection({
  ...
  return (
    <section className="space-y-6">
      <CardV5>
        <h2 className="h3 font-bold mb-2" style={{ color: 'var(--royal-blue)' }}>{headline}</h2>
        <p className="body-sm" style={{ color: 'var(--text-secondary)' }}>{creatorStory}</p>
      </CardV5>

      <div>
        <h3 className="body font-bold mb-3" style={{ color: 'var(--text)' }}>왜 지금인가</h3>
        <div className="grid grid-cols-3 gap-2">
          {[whyNow1, whyNow2, whyNow3].map((t, i) => (
            <div key={i} className="rounded-xl p-3 text-center caption font-medium" style={{ backgroundColor: 'var(--bg-secondary)', color: 'var(--text)' }}>{t}</div>
          ))}
        </div>
      </div>

      <div>
        <h3 className="body font-bold mb-3" style={{ color: 'var(--text)' }}>성장 근거</h3>
        <div className="space-y-2">
          {[growthReason1, growthReason2, growthReason3].map((r, i) => (
            <div key={i} className="flex items-center gap-2 py-2 px-3 rounded-lg" style={{ backgroundColor: 'var(--bg-secondary)' }}>
              ...
              <span className="body-sm" style={{ color: 'var(--text-secondary)' }}>{r}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <CardV5>
        <h3 className="body-sm font-bold mb-3" style={{ color: 'var(--text)' }}>배당 구조</h3>
        ...
        <p className="caption mt-2" style={{ color: 'var(--text-muted)' }}>월 매출의 30%를 배당 풀에 적립 후, 보유 지분 비율에 따라 매월 분배</p>
      </CardV5>

      <div>
        <h3 className="body font-bold mb-3" style={{ color: 'var(--text)' }}>자주 묻는 질문</h3>
          ...
          {FAQ.map((f, i) => (
            <div key={i} className="rounded-xl border overflow-hidden" style={{ borderColor: 'var(--border)' }}>
              <button
                ...
                className="w-full flex items-center justify-between px-4 py-3 text-left body-sm font-medium"
              ...
              {openFaq === i && (
                <div className="px-4 pb-3 caption" style={{ color: 'var(--text-secondary)' }}>{f.a}</div>
              )}
            </div>
          ))}
        </div>
