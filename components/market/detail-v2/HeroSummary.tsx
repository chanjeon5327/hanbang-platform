...
export default function HeroSummary({ item }: Props) {
  ...
  return (
    <section className="px-4 pt-4">
      <div
        className={[
          'relative overflow-hidden rounded-[32px] border border-white/10 shadow-[0_22px_70px_rgba(0,0,0,0.42)]',
          tone.cardClass,
        ].join(' ')}
      >
        <div className={`pointer-events-none absolute -right-10 -top-10 h-36 w-36 rounded-full blur-3xl ${tone.haloClass}`} />
        <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(180deg,rgba(255,255,255,0.04),transparent_34%)]" />

        <div className="relative p-5">
          <div className="flex items-start justify-between gap-3">
            <div className={`inline-flex rounded-full border px-3 py-1 text-[11px] font-medium tracking-[0.02em] ${tone.chipClass}`}>
          ...
          <div className="mt-4 flex gap-4">
            <div className="relative h-24 w-24 shrink-0 overflow-hidden rounded-[28px] border border-white/10 bg-white/[0.06] shadow-[0_10px_30px_rgba(0,0,0,0.28)]">
              {thumbnail ? (
            ...
            <div className="min-w-0 flex-1">
              <h1 className="text-[24px] font-semibold leading-[1.18] tracking-[-0.02em] text-white">
                {title}
              </h1>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2">
                ...
                <span className={`rounded-full border px-3 py-1 text-[12px] ${tone.chipClass}`}>
                  {tone.summaryTag}
                </span>
              </div>
          ...
          <div className="mt-5 rounded-[24px] border border-white/10 bg-black/20 p-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="text-[11px] text-zinc-400">{tone.marketHint}</span>
            </div>
          ...
          <div className="mt-5 rounded-[24px] border border-white/10 bg-white/[0.04] p-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="text-[11px] text-zinc-400">핵심 2가지</span>
            </div>
