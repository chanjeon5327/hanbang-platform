      ...
      {isLoggedIn && !summaryLoading && investSummary && (
        <div className="rounded-[16px] p-4 border card" style={{ backgroundColor: 'var(--card)', borderColor: 'var(--border)' }}>
          <div className="grid grid-cols-3 gap-4 text-center">
            <div>
              <div className="caption font-medium" style={{ color: ROYAL.secondary }}>총 투자금</div>
              <div className="body-sm font-bold tabular-nums mt-0.5 metric" style={{ color: 'var(--text)' }}>
                {formatKrw(investSummary.totalInvest)}
              </div>
            </div>
            <div>
              <div className="caption font-medium" style={{ color: ROYAL.secondary }}>예상 배당 수익률</div>
              <div className="body-sm font-bold tabular-nums mt-0.5 metric" style={{ color: investSummary.unrealizedRate >= 0 ? ROYAL.positive : 'var(--accent-loss)' }}>
                {investSummary.unrealizedRate >= 0 ? '+' : ''}{formatRate(investSummary.unrealizedRate)}
              </div>
            </div>
            <div>
              <div className="caption font-medium" style={{ color: ROYAL.secondary }}>이번 달 배당</div>
              <div className="body-sm font-bold tabular-nums mt-0.5 metric text-profit">
