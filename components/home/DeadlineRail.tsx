  ...
  return (
    ...
    >
      <div className="relative overflow-hidden rounded-2xl border border-black/5 active:scale-[0.97] transition-all duration-200" style={{ backgroundColor: 'var(--toss-card)', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
        ...
        <div className="p-3">
          <div className="body-sm font-bold line-clamp-1 leading-snug" style={{ color: 'var(--toss-text)' }}>{item.title}</div>
          {item.creator_name && <div className="caption mt-0.5 font-medium truncate" style={{ color: 'var(--toss-text-secondary)' }}>{item.creator_name}</div>}
        </div>
      </div>
    </Link>
  );
  ...
  return (
    <section className="mb-6" aria-label="마감임박" data-testid="deadline-rail">
      ...
      {loading ? (
        ...
        <p className="body-sm py-6 text-center" style={{ color: 'var(--toss-text-secondary)' }}>마감 예정인 작품이 없습니다.</p>
      )}
    </section>
  );
}
