-- rpc_invest_and_notify: p_content_id 기준으로 전환
-- 입력: p_content_id (content_items.id)
-- orders: content_id 저장, product_id는 products에서 매핑 시 병행
...
-- 기존 p_product_id 시그니처 오버로드 유지 (content_id와 동일 UUID로 호출 시)
DROP FUNCTION IF EXISTS public.rpc_invest_and_notify(uuid, uuid, numeric);
CREATE OR REPLACE FUNCTION public.rpc_invest_and_notify(
  p_user_id uuid,
  p_product_id uuid,
  p_amount_krw numeric
...
AS $$
BEGIN
  RETURN public.rpc_invest_and_notify(p_user_id, p_product_id, p_amount_krw, NULL::text);
END;
$$;

GRANT EXECUTE ON FUNCTION public.rpc_invest_and_notify(uuid, uuid, numeric, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.rpc_invest_and_notify(uuid, uuid, numeric, text) TO service_role;
