...
const FALLBACK_RAILS: Rail[] = [
  { key: 'fallback-top', title: '오늘의 추천', items: [{ id: 'sample-1', title: '여행가 제이', creator_name: '유튜브' }, { id: 'sample-2', title: '먹방 로드', creator_name: '유튜브' }, { id: 'sample-3', title: '일상 브이로그', creator_name: '유튜브' }, { id: 'sample-4', title: '웹툰 작가 A', creator_name: '웹툰' }, { id: 'sample-5', title: '웹소설 작가 B', creator_name: '웹소설' }] },
  { key: 'fallback-hot', title: '마감 임박', items: [{ id: 'sample-6', title: '뮤직 비디오 프로젝트', creator_name: '음악' }, { id: 'sample-7', title: '드라마 리메이크', creator_name: 'OTT' }, { id: 'sample-8', title: '팟캐스트 시즌2', creator_name: '오디오' }] },
  { key: 'fallback-interest', title: '인기 수익권', items: [{ id: 'sample-1', title: '여행가 제이', creator_name: '유튜브' }, { id: 'sample-2', title: '먹방 로드', creator_name: '유튜브' }, { id: 'sample-3', title: '일상 브이로그', creator_name: '유튜브' }] },
];
...
function RailCard({ item, index, badge }: { item: RailItem; index: number; badge?: string }) {
  const hues = ['#3182f6', '#10b981', '#8b5cf6', '#f59e0b', '#ec4899'];
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden rounded-2xl border border-black/5 active:scale-[0.97] transition-all duration-200" style={{ backgroundColor: 'var(--toss-card)', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
        <div className="aspect-[4/5] relative overflow-hidden" style={{ background: `linear-gradient(135deg, ${hue}22 0%, transparent 100%)` }}>
          {item.thumbnail_url ? (
            <img src={item.thumbnail_url} alt="" className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-105" loading="lazy" />
          ) : (
            <div className="absolute inset-0" style={{ background: `linear-gradient(135deg, ${hue}33 0%, transparent 60%)` }} aria-hidden />
          )}
    ...
    <section className="mb-6">
      ...
      <div className="flex overflow-x-auto no-scrollbar pb-2 -mx-1" style={{ gap: RAIL_GAP }}>
        {rail.items.map((item, i) => (
          <RailCard key={`${item.id}-${i}`} item={item} index={i} badge={badge} />
        ))}
      </div>
    </section>
  );
    ...
    fetch('/api/home/rails', { cache: 'no-store' })
      ...
      .then((json) => {
        if (json?.rails?.length > 0) {
          ...
          if (firstItem) {
            setHeroItem({ id: firstItem.id, title: firstItem.title, subtitle: firstItem.creator_name ? `${firstItem.creator_name} · 수익권` : undefined, thumbUrl: firstItem.thumbnail_url, score: firstItem.score });
          }
        }
      })
      .catch(() => {})
    ...
    <div className="min-h-screen pb-24" style={{ backgroundColor: 'var(--toss-bg)' }}>
      <main className="max-w-lg mx-auto px-4 pt-4 pb-6 space-y-4">
        ...
        {loading ? (
          ...
          displayRails.map((rail) => <RailSection key={rail.key} rail={rail} />)
        )}
