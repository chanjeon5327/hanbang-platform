...
export default function MarketDetailPage() {
  ...
  const summarySection = (
    <div id="market-price-card" className="rounded-2xl border border-black/10 bg-white p-5 shadow-[0_6px_20px_rgba(0,0,0,0.05)]">
  ...
  const orderBookSection = <OrderBookMiniUpbit assetId={id} basePrice={price} />;
  const orderPanelSection = <TradePanelUpbit assetId={id} basePrice={price} />;
  ...
  const tabContentSection = (
    <>
      ...
      {tab === 'price' && (
        <p className="text-sm text-black/65">위 차트·체결·호가에서 실시간 가격을 확인할 수 있습니다.</p>
      )}
      {tab === 'trade' && (
        <p className="text-sm text-black/65">위 주문하기에서 매수/매도 주문을 넣을 수 있습니다.</p>
      )}
    </>
  );
  ...
  return (
    <div className="min-h-screen bg-[#F7F8FA] text-[#0B1120]">
      {/* 모바일: 스캐폴드 고정 순서 (차트 → 체결 → 호가 → 주문 → 탭 콘텐츠) */}
      <div className="block lg:hidden">
        <header className="max-w-7xl mx-auto px-5 sm:px-6 pt-8 pb-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h1 className="mt-1 text-2xl font-extrabold tracking-[-0.4px]">{title}</h1>
            </div>
        ...
        <main className="max-w-7xl mx-auto pb-14 min-h-screen">
          {!item ? (
            <div className="mx-4 mb-5 rounded-2xl border border-amber-300/40 bg-amber-100 p-5 text-amber-900">
      ...
      <div className="hidden lg:block">
        <header className="max-w-7xl mx-auto px-5 sm:px-6 pt-8 pb-4">
          ...
          <div className="mt-5">{summarySection}</div>
        </header>
