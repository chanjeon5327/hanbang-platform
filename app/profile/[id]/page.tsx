    ...
    return (
      <div className="pb-24" style={{ backgroundColor: 'var(--toss-bg)' }}>
        <header className="sticky top-0 z-10 flex items-center gap-3 px-4 py-3 border-b" style={{ backgroundColor: 'var(--toss-card)', borderColor: 'var(--toss-border)' }}>
          ...
          <span className="body-lg font-bold" style={{ color: 'var(--toss-text)' }}>프로필</span>
        </header>
    ...
    <div className="pb-24" style={{ backgroundColor: 'var(--toss-bg)' }}>
      ...
      <div className="max-w-lg mx-auto px-4 py-8">
        <div className="flex flex-col items-center">
          ...
          <h1 className="h3 font-bold mt-4" style={{ color: 'var(--toss-text)' }}>{p.nickname}</h1>
          {p.role && p.role !== 'USER' && (
            <span className="caption mt-1 px-2 py-0.5 rounded" style={{ backgroundColor: 'var(--toss-blue)', color: '#fff' }}>
              {p.role}
            </span>
          )}
        </div>
        ...
        <div className="mt-8 space-y-4">
          {user?.id === id && (artistContributions.length > 0 || artistProgress.length > 0) && (
            <section className="rounded-2xl p-4 border" style={{ backgroundColor: 'var(--toss-card)', borderColor: 'var(--toss-border)' }}>
              <h2 className="body font-semibold mb-3" style={{ color: 'var(--toss-text)' }}>공식 파트너십</h2>
          ...
          <section className="rounded-2xl p-4 border" style={{ backgroundColor: 'var(--toss-card)', borderColor: 'var(--toss-border)' }}>
            <h2 className="body font-semibold mb-2" style={{ color: 'var(--toss-text)' }}>최근 활동</h2>
            <p className="body-sm" style={{ color: 'var(--toss-text-secondary)' }}>준비 중입니다.</p>
          </section>
          <section className="rounded-2xl p-4 border" style={{ backgroundColor: 'var(--toss-card)', borderColor: 'var(--toss-border)' }}>
            <h2 className="body font-semibold mb-2" style={{ color: 'var(--toss-text)' }}>관심작품</h2>
            <p className="body-sm" style={{ color: 'var(--toss-text-secondary)' }}>준비 중입니다.</p>
          </section>
        </div>
      </div>
